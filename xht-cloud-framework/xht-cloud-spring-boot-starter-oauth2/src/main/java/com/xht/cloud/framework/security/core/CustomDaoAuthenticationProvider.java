...
/**
 * 描述 ：重写{@link DaoAuthenticationProvider}
 *
 * @author : 小糊涂
 **/
...
public class CustomDaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    protected final UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken authentication)
        ...
        try {
            Map<String, IUserDetailsService> userDetailsServiceMap = SpringContextUtil
                    .getBeansOfType(IUserDetailsService.class);
            IUserDetailsService iUserDetailsService = userDetailsServiceMap.values()
                    .stream()
                    .filter(service -> service.support(CustomAuthorizationGrantType.PASSWORD.getValue(), null)).findFirst().orElse(null);
            if (iUserDetailsService == null) {
                throw new InternalAuthenticationServiceException(
                        "UserDetailsService returned null, which is an interface contract violation");
            }
            UserDetails loadedUser = iUserDetailsService.loadUserByUsername(username);
            if (loadedUser == null) {
                throw new InternalAuthenticationServiceException(
                        "UserDetailsService returned null, which is an interface contract violation");
            }
